...
//		 }
	...
	public static void createTable(String tablename, String key, Hashtable<String, String> columnsnametype)
			throws DBAppException, IOException {//Creates the table folder and initial meta data in our storage
		emptyHash(columnsnametype);//checks if the input is empty or not  
		if (!columnsnametype.containsKey(key))//checks if the primary key was in the input fields 
		...
		for (int i = 0; i < inputHashtablecontent.size(); i++) {
			//inputHashtablecontentString holds the ith element in the input hashtable
			...
			if (inputHashtablecontentString.equalsIgnoreCase("java.lang.Integer") || inputHashtablecontentString.equalsIgnoreCase("java.lang.String")
					|| inputHashtablecontentString.equalsIgnoreCase("java.lang.Double") || inputHashtablecontentString.equalsIgnoreCase("java.lang.Boolean")
		...
		//dir is the directory of the new folder that will be created for the new table
	...
	public static void loadCSV(Hashtable<String, String> columnsnametype, String tablename, String pk)
		...
		//pw is a PrintWriter that appends the new data to the end of the file instead of overwriting it
		PrintWriter pw = new PrintWriter(new FileWriter("metadata.csv", true));
		StringBuilder sb = new StringBuilder();//sb is the new data that will be appended to our metadata storage
		...
		//appends the type of the not know to user column that holds the date of the last update
	...
	public static PriorityQueue<Tuple> Readfiles(String strTableName) throws IOException, DBAppException{
		...
		//Tuple is data structure that we have created that is an arraylist that implements comparable so we can compare the primary key easily
		//Because Tuple implements comparable, we have decided to use priority queues to have the data automatically sorted
		//table holds all of the data that we have in the stored files in a 2D form
		PriorityQueue<Tuple> table = new PriorityQueue<Tuple>();
		//tableOfTables is a 3D data structure that holds the pieces of the data that we have stored in the database with each index holding one page
		//for example if we have 3 pages and each page has 200 rows, then tableOfTables' size will be 3 with each index holding a 2D array that holds one page
		//Nb. we consider the PriorityQueue as a normal array in this documentation to ease the explanation
		ArrayList<PriorityQueue<Tuple>> tableOfTables = new ArrayList<PriorityQueue<Tuple>>();
		try {
			//path is the directory of the stored pages, we start with 1 as its the first entry in our storage system
			String path = strTableName + File.separator + "Page" + 1 + ".class";
			//fileIn reads the file from the path
			FileInputStream fileIn = new FileInputStream(path);
			ObjectInputStream in = null;
			//the condition checks if the file exists or is that table non existent in the first place 
			if (fileIn.available() != 0) {
				//we start the loop with i=2 because we will fetch the first file first then we start looping
				...
				for (int i = 2;; i++) {
					...
					if (fileIn.available() != 0) {
						...
						in = new ObjectInputStream(fileIn);
						//we use a tmp 2D array that we will store in tableOfTables
						PriorityQueue<Tuple> tableTmp = (PriorityQueue<Tuple>) in.readObject();
						tableOfTables.add(tableTmp);
					}
					...
					if (f.isFile() && f.canRead()) {
						//loads the next page
						fileIn = new FileInputStream(path);
					} else //the pages have ended, so we break out of the reading process
						break;
					// printTuples(table);
				}
				in.close();
				...
				System.out.println("tableOfTables size : " + tableOfTables.size());
				//loads the content of the 3D array (tableOfTables) into the 2D array (table) to ease the process of handling arrays
				//as manipulating a 2D array is easier than 3D array and will also decrease the time & space complexity of the program
				for (int i = 0; i < tableOfTables.size(); i++) {
					int queueInitialSize = tableOfTables.get(i).size();
					for (int j = 0; j < queueInitialSize; j++) {
						table.add(tableOfTables.get(i).remove());
	...
	public static void WriteFile(String strTableName,PriorityQueue<Tuple> table, int n){
		...
		try {
			...
			//tableInitialSizeModN is the number of pages that the table should have
			...
			for (int i = 0; i <= tableInitialSizeModN; i++) {
				...
				//tableTmp is the valuable that holds the part of the data that should be stored next
				//for example, if i=3, n=200, and tableInitialSizeModN=4 then tabletmp should hold
				...
				for (int k = 0; ((k % n != 0 || k == 0) && !(table.isEmpty())); k++) {
					//we add the data from table to tableTmp so we can store it later in our storage system
	...
	public static ArrayList loadHashtableInput(Hashtable<String, Object> htblColNameValue,ArrayList<String> metadataArray) throws DBAppException{
		//tmpInteger stores the position of the primary key
		int tmpInteger = -1;
		//value stores the data of the key for a new row
		Object value = null;
		//tuple is the new arraylist that will hold the value of the row that needs to be insearted
		Tuple tuple = new Tuple();
		//trioTuple is a tuple that holds the results of (tuple,tmpInteger,value)
		//we have used this way so we can return all of the three value to the main program
		...
		while (enumKey.hasMoreElements()) {
			...
			String valString = val.getClass().toString();// gets the type of the field
			String[] typeArray = valString.split(" ");
			//flag checks if the column name is valid or not
			boolean flag = false;
			for (int i = 0; i < metadataArray.size(); i++) {
				//metaArray holds the type of each column from the metadata file that we have in our storage
				String[] metaArray = metadataArray.get(i).split(",");
				//the conditions makes sure that the type of the field matches the one in the metadata
	...
	public static void insertIntoTable(String strTableName, Hashtable<String, Object> htblColNameValue)
		...
		tuple.setKey(value); // after checking that its a new key I'm setting, its value in tuple
		table.add(tuple);
